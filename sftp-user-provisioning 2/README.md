...
- **Vault access is only granted to the approved job.** The Vault role is bound to
  the `sftp-production` environment, so an unapproved run cannot write secrets.
...
### 3. Vault: JWT auth for GitHub Actions
...
# what the pipeline may do with the secrets
...
The KV v2 engine must exist at `secret/` (or set `VAULT_KV_MOUNT`). Each user
ends up at `secret/sftp/<user_name>` with the fields `private_key` and `public_key`.
...
No secrets are needed: AWS and Vault both use GitHub's OIDC token.
...
| Vault login `permission denied` | `bound_subject` / `bound_audiences` on the JWT role don't match, or Vault can't reach GitHub's discovery URL. |
