...
#                ALLOW_DISK_KEYS=1 (local testing on macOS/Git Bash, see below)
...
# Everything sensitive lives in RAM (/dev/shm) and is wiped on exit. macOS and
# Git Bash have no /dev/shm: for LOCAL TESTING ONLY you can set ALLOW_DISK_KEYS=1
# to use a normal temp dir instead (the keys are still deleted on exit).
if [[ -d /dev/shm ]]; then
  KEY_BASE=/dev/shm
elif [[ "${ALLOW_DISK_KEYS:-0}" == "1" ]]; then
  KEY_BASE="${TMPDIR:-/tmp}"
  KEY_BASE="${KEY_BASE%/}"
  log "WARNING: no /dev/shm, keys will briefly be written to $KEY_BASE (local testing only)"
else
  die "/dev/shm not found (macOS / Git Bash?). Run on Linux or WSL, or for local testing only set ALLOW_DISK_KEYS=1"
fi
